...
import { deleteMod, getMyMods } from "../../../shared/mods/api/requests.ts";
...
import { useConfirmStore } from "../../../shared/ui/model/useConfirmStore.ts";
...
export const ProfilePage = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (modId: string) => deleteMod(modId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['myMods'] });
    },
    onError: (e) => {
      console.error('[FRONT] Не удалось удалить мод:', e);
      alert('[FRONT] Ошибка при удалении');
    }
  })

  const handleDeleteMod = (modId: string, title: string) => {
    openConfirm(
      'Удаление мода',
      `Вы действительно хотите удалить мод "${title}"?`,
  ...
  return (
    <section className={styles.mainContainer}>
      <div className={styles.profileContainer}>
        ...
        <div className={styles.myModsTitleContainer}>
        ...
        <div className={styles.modsList}>
          {
            myMods?.map((mod: Mod) => (
              <div className={styles.modContainer} key={mod.id}>
